...
import { Search,Home,LayoutGrid, User,X, MessageSquare, Heart,AlignJustify, ShoppingCart, Menu, ChevronDown } from 'lucide-react';
        ...
        <div className="flex items-center gap-4 lg:gap-6 shrink-0">
          ...
          {localStorage.getItem('user') ? (
            <Link to="/profile" className="flex flex-col items-center text-gray-500 hover:text-blue-600">
                ...
                <span className="text-xs hidden md:block mt-1">Profile</span>
            </Link>
          ) : (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto py-4">
              <div className="px-4 space-y-2">
                <Link to="/" className="flex items-center gap-3 py-2 text-gray-700 font-medium">
                  <div className="w-6 h-6 flex items-center justify-center"><Home className="w-5 h-5" /></div>
                  Home
                </Link>
                <Link to="/categories" className="flex items-center gap-3 py-2 text-gray-700 font-medium">
                  <div className="w-6 h-6 flex items-center justify-center"><LayoutGrid className="w-5 h-5" /></div>
                  All categories
                </Link>
                <Link to="/favorites" className="flex items-center gap-3 py-2 text-gray-700 font-medium border-b border-gray-100 pb-4">
                  <div className="w-6 h-6 flex items-center justify-center"><Heart className="w-5 h-5" /></div>
                  Favorites
                </Link>
                
                <div className="pt-4 space-y-4">
                  <Link to="/hot-offers" className="block py-1 text-gray-600">Hot offers</Link>
                  <Link to="/gift-boxes" className="block py-1 text-gray-600">Gift boxes</Link>
                  <Link to="/projects" className="block py-1 text-gray-600">Projects</Link>
                  <Link to="/menu-item" className="block py-1 text-gray-600">Menu item</Link>
                </div>
              </div>
