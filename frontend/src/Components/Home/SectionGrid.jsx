...
const SectionGrid = ({ title, bgImage, items }) => {
    return (
        <div className="container mx-auto px-4 py-4">
            <div className="bg-white border border-gray-200 rounded-md flex flex-col lg:flex-row h-auto overflow-hidden">
                {/* Banner Side (Desktop) */}
                <div 
                ...
                {/* Grid Side */}
                <div className="flex-1 grid grid-cols-2 lg:grid-cols-4 gap-px bg-gray-100 h-full w-full">
                     {items.slice(0, 8).map((item, idx) => (
                         <Link to={`/product/${item._id}`} key={idx} className="p-3 md:p-4 flex flex-col-reverse lg:flex-row justify-between lg:items-center h-auto lg:h-[130px] bg-white group border-r lg:border-b-0 border-gray-100 gap-2 lg:gap-0">
                             <div className="flex flex-col h-full justify-start pt-1 min-w-0 lg:pr-2 w-full">
                ...
                <div className="lg:hidden px-4 py-3 border-t border-gray-100 bg-white">
                    <Link to="/categories" className="text-blue-600 font-medium text-sm flex items-center gap-2">
                        Source now
